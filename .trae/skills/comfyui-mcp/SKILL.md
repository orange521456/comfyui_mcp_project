---
name: "comfyui-mcp"
description: "Control local ComfyUI for image generation via MCP tools. Invoke when user asks to generate images, build/modify ComfyUI workflows, manage models, or any ComfyUI-related tasks."
---
...
2. **每张图一个工作流**：每个工作流只生成一张图。需要多张图时，Agent 用大模型语义拆分用户输入，循环调用 `build_workflow` → `execute_and_watch` → `get_generated_image`。
...
3. **循环生成**：对每个子 prompt：
   ```
   build_workflow(pipeline_ir) → execute_and_watch() → get_generated_image()
   ```
   - `execute_and_watch` 通过 WebSocket 实时监控执行，自动等待完成并返回结果
   - 等待每张图完成后再开始下一张
   - 汇总所有结果后统一返回给用户
...
3. execute_and_watch()  → 自动等待执行完成，返回 prompt_id 和结果
4. get_generated_image(filename)  → 返回缩略图给用户
```
...
3. 对每个子 prompt 循环执行 build_workflow → execute_and_watch → get_image
4. 汇总三张图片返回给用户
```
